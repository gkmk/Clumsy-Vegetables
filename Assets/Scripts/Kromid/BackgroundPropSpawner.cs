	...
	IEnumerator Spawn ()
	{
		...
		while(propInstance != null)
		{
			...
			if(facingLeft)
			{
				...
				if(propInstance.transform.position.x < transform.position.x +leftSpawnPosX - 0.5f)
					// ... destroy the prop.
			...
			else
			{
				...
				if(propInstance.transform.position.x > transform.position.x+rightSpawnPosX + 0.5f)
					// ... destroy the prop.
					Destroy(propInstance.gameObject);
			}
